...
blocked_files="$(git ls-files | grep -Ei '(^|/)(pb_data|pb_backups|private_data|runtime_uploads|ocr_output|exports?|backups?|logs?)/|\.(env|sqlite|sqlite3|db|csv|tsv|xlsx?|pdf|log|pem|key|p12|pfx)$' || true)"
